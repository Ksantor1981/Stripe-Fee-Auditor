...
function SavingsOpportunityRow({ opp, index }: { opp: SavingsOpportunity; index: number }) {
  ...
  const title = localized?.title ?? opp.title;
  const tip = localized?.tip ?? opp.tip;
  const actionLabel = localized?.actionLabel ?? opp.actionLabel;
  const periodLossNote = localized?.periodLossNote ?? opp.periodLossNote;
  const annualSavingsNote = localized?.annualSavingsNote ?? opp.annualSavingsNote;
  const steps = localized?.steps ?? opp.steps;
    ...
    <div className={`rounded-xl border px-4 py-4 ${style.border} ${style.bg}`}>
      ...
      {opp.periodLoss != null && opp.periodLoss > 0 && (
        <p className="text-xs font-medium text-amber-800 ml-7 mb-2">
          ...
          {periodLossNote ? ` (${periodLossNote})` : ` (${tc("estimate")})`}
        </p>
      )}

      <p className="text-xs text-gray-600 leading-relaxed ml-7 mb-3">{tip}</p>

      {(opp.kind === "ach_large_invoices" || /ACH/i.test(opp.title)) && (
    ...
    <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
      ...
      <div className="space-y-3">
        {opportunities.map((opp, i) => (
          <SavingsOpportunityRow key={i} opp={opp} index={i} />
        ))}
      </div>
    </div>
  );
